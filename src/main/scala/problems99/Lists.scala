...
object Lists {
  ...
  def removeAt[A]: ((Int, List[A])) => (List[A], Option[A]) = {
    case (_, Nil) => (Nil, None)
    case (i, head::tail) if i <= 0 => (tail, Some(head)) 
    case (i, head::tail) => removeAt(i-1, tail) match {
      case (rest, removed) => (head::rest, removed)
    }
  }
  ...
  /** P22 (*) Create a list containing all integers within a given range.
    ...
    */
  def range: (Int, Int) => List[Int] = (i: Int, j: Int) => (i, j) match {
    case _ if i > j => Nil 
  ...
  def randomIndex[A](l: List[A]): (Int, List[A]) = (random.nextInt(l.length), l)
  def randomSelect[A]: (Int, List[A]) => List[A] = {
    case (n, Nil) => Nil
    case (n, l) => {
      removeAt[A].compose(randomIndex[A])(l) match {
        case (rest, Some(removed)) => removed :: randomSelect(n-1, rest)
        case (rest, None) => throw new Exception("This should never happen")
      }
    }
  }
  ...
  def lotto: (Int, Int) => List[Int] = {
    case (i, _) if i <= 0 => Nil
    case (i, j) =>  random.nextInt(j) + 1 :: lotto(i-1, j)
  }
  ...
  def randomPermute[A]: List[A] => List[A] = {
    case l => randomSelect(l.length, l)
  }
  ...
  def combinations[A]: (Int, List[A]) => List[List[A]] = {
    case (0, _) => List(Nil)
    case (_, Nil) => Nil
    case (k, l) if k == 1 => l.map(List(_)) 
    case (k, l) if k == l.length => List(l)
